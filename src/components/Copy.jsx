...
const Copy = () => {
 ...
 <div
      ...
      <button
        onClick={handleCopy}
        className="flex items-center w-[100%] gap-2 md:w-[100%] xl:rounded-[1.4vw] h-[43%]   relative overflow-hidden xl:h-[37%] rounded-[4vw] md:rounded-[2vw] md:h-[10vw] md:text-[2.5vw]  xs:w-[100%] xs:h-[18vw] xs:text-[5vw] text-[1rem] lg:text-[1rem] xl:text-[0.9rem] lg:h-[30%]"
      >
        <div className="w-full h-full flex items-center justify-center relative z-[4] text-[#363636] bg-gray-100 lg:dark:bg-transparent  dark:bg-zinc-900 dark:text-[#cbcbcb]  gap-2 ">
          {copyState ? (
            ...
            <>
              <h2
                className={`${currentTa && "xl:text-[0.5rem] xs:text-[3vw] text-[0.8rem] md:text-[0.6rem] lg:text-[0.8rem]" } ${currentUr && "xl:text-[0.7rem] xs:text-[4vw] text-[0.9rem] md:text-[0.7rem] lg:text-[0.9rem]" } xs:text-[4vw] ${
                  isTransitioning ? "opacity-0" : "opacity-1"
